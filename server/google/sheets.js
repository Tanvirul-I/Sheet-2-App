 ...
 * @return {obj}
 */
let readSheet = async (spreadsheetId, range, GID, majorDimension = "COLUMNS") => {
    const oauth2Client = OAuthObjectStore.getMain(); // gets the main OAuth Client Object
    const service = google.sheets({ version: "v4", auth: oauth2Client });
...
let checkCache = async (type, spreadsheetId, GID) => {
    ...
    if (cacheSpreadsheet && cacheSheet && !cache.compare(spreadsheetId, checkSheet)) {
        console.log("Spreadsheet has been UPDATED");
        cache.delete(spreadsheetId);
        return { success: false, data: null, modifiedData: checkSheet };
    }

    if (!cacheSpreadsheet) return { success: false, data: cacheSheet, modifiedData: checkSheet };
    else if (type == "spreadsheet")
        return { success: true, data: cacheSheet, modifiedData: checkSheet };

    if (!cacheSheet) return { success: false, data: cacheSheet, modifiedData: checkSheet };
    else if (type == "sheet") return { success: true, data: cacheSheet, modifiedData: checkSheet };
